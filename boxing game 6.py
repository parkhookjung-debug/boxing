...
# ══════════════════════════════════════════════════════════════════
POSE_INPUT_MAX = 320
POSE_TARGET_FPS = 15.0
POSE_MIN_INTERVAL = 1.0 / POSE_TARGET_FPS
...
class LitePoseModel:
    def __init__(self):
        ...
        lite_path = os.path.join(base_dir, "pose_landmarker_lite.task")
        full_path = os.path.join(base_dir, "pose_landmarker_full.task")
        model_path = lite_path if os.path.exists(lite_path) else full_path
        if not os.path.exists(model_path):
            raise SystemExit("pose_landmarker_lite.task or pose_landmarker_full.task is required")
    ...
    def __call__(self, frame_bgr):
        now = time.time()
        if self.last_kp is not None and now - self.last_infer_t < POSE_MIN_INTERVAL:
            return [self.last_kp.copy()], [self.last_sc.copy()]

        h, w = frame_bgr.shape[:2]
        scale = min(1.0, POSE_INPUT_MAX / max(h, w))
        if scale < 1.0:
        ...
        if not result.pose_landmarks:
            if self.last_kp is not None and now - self.last_seen_t < POSE_CACHE_TTL:
                return [self.last_kp.copy()], [self.last_sc.copy()]
            return [], []
        ...
        for coco_i, mp_i in MP_TO_COCO.items():
            ...
            sc[coco_i] = getattr(lm, "visibility", 1.0)

        if self.last_kp is not None:
            kp = self.last_kp * (1.0 - POSE_SMOOTH_ALPHA) + kp * POSE_SMOOTH_ALPHA

        self.last_seen_t = now
        self.last_kp = kp
        self.last_sc = sc
        return [kp.copy()], [sc.copy()]

print("MediaPipe Pose Lite loading...")
pose_model = LitePoseModel()
print(f"Ready: lightweight pose backend ({pose_model.model_name})\n")
...
cv2.namedWindow('Boxing Game 6', cv2.WINDOW_NORMAL)
cv2.resizeWindow('Boxing Game 6', WIN_W, WIN_H)
...
while cap.isOpened():
    ...
    if _paused and _gstate == 'PLAYING':
        ...
        cv2.imshow('Boxing Game 6', canvas)
